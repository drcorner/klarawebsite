...
export default function Logo({ variant = "default", className = "" }: LogoProps) {
  ...
  const accentColor = variant === "light" ? "text-cream/60" : "text-teal";
  
  return (
    <Link href="/" data-testid="link-logo">
      <div className={`flex items-center gap-2.5 ${className}`}>
        <div className={`w-9 h-9 rounded-xl bg-gradient-to-br from-teal to-teal-dark flex items-center justify-center shadow-sm ${variant === "light" ? "from-cream/20 to-cream/10" : ""}`}>
